...
export class OfficeExpenseHeadComponent {
  ...
  saveOfficeExpenseHead() {
    ...
    var obj: RequestModel = {
      request: this.localService
        .encrypt(JSON.stringify(this.OfficeExpenseHead))
        .toString(),
    };
    this.service.saveOfficeExpenseHead(obj).subscribe(
      (r1) => {
        let response = r1 as any;
        if (response.Message == ConstantData.SuccessMessage) {
          if (this.OfficeExpenseHead.OECategoryId > 0) {
            this.toastr.success(
              'Office Expense head detail updated successfully'
            );
  ...
  deleteOfficeExpenseHead(obj: any) {
    if (confirm('Are your sure you want to delete this recored')) {
      ...
      this.dataLoading = true;
      this.service.deleteOfficeExpenseHead(request).subscribe(
        (r1) => {
          let response = r1 as any;
          if (response.Message == ConstantData.SuccessMessage) {
